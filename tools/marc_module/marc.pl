...
# Input:
#  Argument 1 (required)
#   Folder with files from DRA used in Variations, including all access, preservation, RTF and PDF files.
#
#  Argument 2 (optional)
#   Location you'd like the marc file written. By default, this is the folder of the input
#
#  Argument 3 (optional)
...
#   If no second argument is specified, file in the root of the same directory that is the input.
#
# Description:
#  Input a folder from DRA with wav files, parse contents for metadata, output a
#  text file formatted according to the SirsiDynix flat file format into the
...
#   From "info"
#   260c = creationdate
#   490a = name -- this always seems to be "Indiana University Jacobs School of Music"
#   490v = subject
...
$MDSSFileListingPath = $path . "/access\/logs\/" . $varID . "_MDSSFileNames.txt";
...
if ( $info->{'genre'} =~ m/orchestra|ensemble|choir|singers|trio|quartet|quintet|institute|concentus/i ) {
	...
	print MARC ".856." . " 40" . "|xhttp://purl.dlib.indiana.edu/iudl/variations/sound/" . $varID . "|uOnline copy in process. Not yet available.\n";
