...
sub get_cues {
	...
	$record -> {'ltxt'} = $data -> {'ltxt'} -> {$id} if ( exists $data -> {'ltxt'} -> {$id} );
	$output -> {$id} = $record;
    }
    return $output;
}
...
sub _read_file {
	...
	} elsif ( $head eq 'JUNK' ) {								# Added by TEG to get JUNK
	    $details{'JUNK'} = $self -> _read_raw( $chunk_len );
	    next;
	}  elsif ( $head eq 'bext' ) {								# Added by TEG to get bext
	    $details{'bext'} = $self -> _read_raw( $chunk_len );
	    next;
	} else {
	    $head =~ s/[^\w]+//g;
...
sub _read_list {
	...
	while ( $pos < $length ) {
	    ...
	    if ( $head eq 'ltxt' ) {  					# Changed by TEG to not make an ltxt that is too small
#			my $record = $self -> _decode_block( [ 1 .. 5 ] );  # Commented out since we know the length
