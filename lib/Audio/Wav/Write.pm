...
sub _init {
    ...
    my @wanted = ( 'block_align', 'bytes_sec', 'info', 'wave-ex', 'bext', 'JUNK' );  #TEG added btext and JUNK
...
sub _examine_details {
    my $self = shift;
    my $details = shift;
    my( $cue, $label, $note, $ltxt )
	= map exists( $details -> {$_} ) ? $details -> {$_} : {},
	qw( cue labl note ltxt );
    my $block_align = $self -> {'details'} -> {'block_align'};
    my $tools = $self -> {'tools'};
	...
	my( $in_label, $in_note, $in_ltxt )
	    = map exists( $_ -> {$id} ) ? $_ -> {$id} : '',
	    $label, $note, $ltxt;
	$self -> add_cue( $pos, $in_label, $in_note, $in_ltxt );
    }
