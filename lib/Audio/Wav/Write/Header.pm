...
sub finish {
    ...
    $extra += $self -> _write_bext();
#    $extra += $self -> _write_JUNK();
...
sub _write_list_adtl {
	...
	if ( exists $cue -> {'label'} ) {
	    $adtl{'labl'} -> {$cue_id} = $cue -> {'label'};
#	    $adtl .= $self -> _make_chunk( 'labl', pack( 'V', $cue_id ) . $adtl{'labl'} -> {$cue_id} . "\0" );
	}
	if ( exists $cue -> {'note'} ) {
	    $adtl{'note'} -> {$cue_id}  = $cue -> {'note'};
#	    $adtl .= $self -> _make_chunk( 'note', pack( 'V', $cue_id ) . $adtl{'note'} -> {$cue_id} . "\0" );
	}
	if ( exists $cue -> {'ltxt'} ) {						# Added by TEG
	    $adtl{'ltxt'} -> {$cue_id}  = $cue -> {'ltxt'};
	    $adtl .= $self -> _make_chunk( 'ltxt', pack( 'V', $cue_id ) . $adtl{'ltxt'} -> {$cue_id} . "\0" );

	}
    }

    return 0 unless ( keys %adtl );
    $adtl = 'adtl';									# Removed by TEG. Individual make_chunks above added to replace.
...
sub _write_bext {
    my $self = shift;
    return 0 unless $self -> {'details'} -> {'bext'};
    return $self -> _write_block( 'bext', $self -> {'details'} -> {'bext'} );
}

sub _write_JUNK {
    my $self = shift;
    return 0 unless $self -> {'details'} -> {'JUNK'};
    return $self -> _write_block( 'JUNK', $self -> {'details'} -> {'JUNK'} );
}
